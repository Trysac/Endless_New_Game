...
var fall_damage = 2 #Daño por caida
...
var distance_to_damage:float = 50 # Distancia maxima que se puede caer antes de recibir daño de caida
...
func _move(delta):
	...
	if Input.is_action_just_pressed("saltar"):
		if character.is_on_floor():
			...
			if last_position == 0:
				last_position = character.position.y
			_is_jumping = true
...
func _apply_gravity(delta):
	...
	if _current_movement == _movements.JUMP_WITH_SWORD and not _died:
		...
		if character.is_on_floor():
			...
			_is_jumping = false
			current_position = character.position.y
			_jump_count = 0
			if abs(last_position - current_position) >= distance_to_damage:
				print("current_position : " + str(current_position))
				print("last_position : " + str(last_position))
				print("Es : " + str(abs(last_position - current_position)) + " Mayos igual que : " + str(distance_to_damage) + " Resultado --> " + str(abs(last_position - current_position) >= distance_to_damage))
...
func fall():
	HealthDashboard.remove_life(fall_damage)
